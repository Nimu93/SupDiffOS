...
void page_fault_handler(registers_t* registers) {
    ...
    printf("#PF cr2=0x%x eip=0x%x err=0x%x [%s|%s|%s]\n",
        fault_addr,
        registers->eip,
        error_code,
        (error_code & 0x4) ? "USER" : "KERN",
        (error_code & 0x2) ? "WRITE" : "READ",
        (error_code & 0x1) ? "PROT"  : "NOT-PRESENT");

    if (error_code & 0x1) {
        /* protection violation — page is present but privilege denied; halting */
        asm volatile("cli; hlt");
        return;
    }

    /* not-present + write → allocate on demand */
    if (error_code & 0x2) {
    ...
    /* not-present + read → halt, nothing to recover */
    printf("#PF: unhandled read from non-present page, halting\n");
    asm volatile("cli; hlt");
}
